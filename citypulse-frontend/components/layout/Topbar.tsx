...
export function Topbar() {
  ...
  return (
    <header className="flex flex-col gap-4 border-b border-border bg-surface px-5 py-4 lg:flex-row lg:items-center lg:justify-between">
      <div>
        <p className="text-xs uppercase tracking-[0.28em] text-tx-label">Canlı izleme</p>
        <h2 className="mt-1 text-xl font-semibold text-tx-primary">Akıllı Şehir Dashboard</h2>
      </div>

      <div className="flex flex-wrap items-center gap-3 text-sm text-tx-secondary">
        <span>{sensors.length} aktif sensör</span>
